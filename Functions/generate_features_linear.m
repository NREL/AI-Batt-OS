function [Xout, Xoutvars] = generate_features_linear(X, Xvars)
% Generate calendar storage features for linear models
% Input: 
...
% 4th: add exponential of each feature as a new group
%   Operator B2
Xnew = [];
Xvarsnew = {};
for idx_group = 1:length(X)
    [B2_X,B2_Xvars] = operatorB2(X{idx_group},Xvars{idx_group});
    Xnew = [Xnew,B2_X];
    Xvarsnew = [Xvarsnew,B2_Xvars];
end
X{end+1} = Xnew;
Xvars{end+1} = Xvarsnew;
...
Xoutvars = Xvars(all(isfinite(X),1));
    ...
    function [Xout, Xoutvars] = operatorB2(X, Xvars)
        % Get the exponential of each feature.
        % Input:
        %   X: array of features to transform - each column is a feature
        %   Xvars: cell array of X feature var names
        % Output formatted similary. Total number of output features is N_X.
